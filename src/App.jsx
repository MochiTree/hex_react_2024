  ...
  return (
    <>
     <div className="modal fade" ref={modalRef} tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            </div> { isAdmin ? (<><button className='btn btn-success mb-3 me-2' onClick={backEnd}>{isBackEnd.status ? '產品頁面' : '後台頁面'}</button><button className='btn btn-danger mb-3' onClick={loginCheck}>檢查登入狀態</button><div className="row">
  ...
  <tbody>
    {products.map(function(item){
     return(<tr key={item.id}>
      ...
      <td><button className='btn btn-primary btn-sm' style={{display:`${isBackEnd.display}`}} onClick={showModal}>編輯</button></td>
    </tr>)
    })}
  </tbody>
</table></div>
